...
func TestPRRepo_CreateGetUpdateAndReviewerLookup(t *testing.T) {
	t.Run("given author and reviewer in team when PR is created and merged then PR is found and visible for reviewer", func(t *testing.T) {
		// Arrange
		db := setupTestDB(t)
...
func TestTeamRepo_CreateAndGetByName(t *testing.T) {
	t.Run("given users without team when team is created then team can be read by name and existence check returns true", func(t *testing.T) {
		// Arrange
		db := setupTestDB(t)
		repo := NewPrReviewerServiceRepo(db)
		cleanupDB(t, db)
...
func TestUserRepo_GetByUserIDGetByTeamIDSetIsActive(t *testing.T) {
	t.Run("given team members when querying and deactivating user then reads and status update are persisted", func(t *testing.T) {
		// Arrange
		db := setupTestDB(t)
		repo := NewPrReviewerServiceRepo(db)
		cleanupDB(t, db)
