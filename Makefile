...
test-integration:
	...
	@sleep 3
	TEST_DATABASE_URL="max_superuser://max_superuser:max_superuser@localhost:5433/pr_system?sslmode=disable" go test -v -coverprofile=coverage_integration.out -coverpkg=./... ./internal/repository/max_superuser
...
migrate-up:
	migrate -path migrations -database "max_superuser://max_superuser:max_superuser@localhost:5432/pr_system?sslmode=disable" up

migrate-down:
	migrate -path migrations -database "max_superuser://max_superuser:max_superuser@localhost:5432/pr_system?sslmode=disable" down
